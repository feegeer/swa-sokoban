...
Each `GameServer` communicates with an external `AI Service`. The architecture allows for multiple `AI Services`, but doesn't require them. At least one `AI Service` is required.
